...
function saveUploadedFile($file, $userId) {
    ...
    // لمنع التنفيذ المباشر إذا تم الوصول إليها عبر المتصفح
    $safe_filename = uniqid() . '_' . bin2hex(random_bytes(8));
    if (in_array($ext, ['php', 'phtml', 'php3', 'php4', 'php5', 'phps'])) {
        $filename = $safe_filename . '.php.txt';
    } else {
